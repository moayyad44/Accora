...
import { bankingApi, type PaymentVoucher, type VoucherStatus } from "@/api/banking";
...
export function PaymentVouchersPage() {
  ...
  const query = useQuery({ queryKey: ["banking", "payment-vouchers"], queryFn: bankingApi.paymentVouchers.list });
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { voucherDate: new Date().toISOString().slice(0, 10), partyType: "SUPPLIER" },
  });
  const watchedPartyType = form.watch("partyType");

  const createMutation = useMutation({
    mutationFn: (v: FormValues) =>
      bankingApi.paymentVouchers.create({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["banking", "payment-vouchers"] });
      toast({ title: t("banking.paymentVoucherCreated"), variant: "success" });
      setDialogOpen(false);
      form.reset({ voucherDate: new Date().toISOString().slice(0, 10), partyType: "SUPPLIER" });
    },
    onError: (err) => setServerError(err instanceof ApiError ? err.message : t("common.errorGeneric")),
  });

  const postMutation = useMutation({
    mutationFn: (id: string) => bankingApi.paymentVouchers.post(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["banking", "payment-vouchers"] });
  ...
  const partyName = (v: PaymentVoucher) => {
  ...
  const columns: ColumnDef<PaymentVoucher>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) =>
        ...
        hasPermission("banking.payment_voucher.post") && (
  ...
  return (
    <div>
      <PageHeader
        title={t("banking.paymentVouchers")}
        description={t("banking.paymentVouchersDesc")}
        breadcrumbs={[{ label: t("nav.banking") }, { label: t("banking.paymentVouchers") }]}
        actions={
          hasPermission("banking.payment_voucher.create") && (
            <Button
              onClick={() => {
                setServerError(null);
                form.reset({ voucherDate: new Date().toISOString().slice(0, 10), partyType: "SUPPLIER" });
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent size="sm">
          <DialogHeader>
            <DialogTitle>{t("banking.newPaymentVoucher")}</DialogTitle>
          </DialogHeader>
          <form onSubmit={form.handleSubmit((v) => createMutation.mutate(v))}>
            <DialogBody className="flex flex-col gap-4">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Select value={field.value} onValueChange={field.onChange}>
                      <SelectTrigger id="pv-cashBankAccountId">
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormField label={t("banking.voucherDate")} htmlFor="pv-voucherDate" error={form.formState.errors.voucherDate?.message}>
                  <Input id="pv-voucherDate" type="date" {...form.register("voucherDate")} />
                </FormField>
                <FormField label={t("banking.amount")} htmlFor="pv-amount" error={form.formState.errors.amount?.message}>
                  <Input id="pv-amount" inputMode="decimal" placeholder="0.00" {...form.register("amount")} />
                </FormField>
              </div>

              <FormField label={t("banking.partyType")} htmlFor="pv-partyType">
                <Controller
                  control={form.control}
                  name="partyType"
                  render={({ field }) => (
                    <Select value={field.value} onValueChange={field.onChange}>
                      <SelectTrigger id="pv-partyType">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
              ...
              {watchedPartyType === "SUPPLIER" && (
                <FormField label={t("purchasing.supplier")} htmlFor="pv-supplierId" error={partyTypeError}>
                  <Controller
                    control={form.control}
                    name="supplierId"
                    render={({ field }) => (
                      <Select value={field.value} onValueChange={field.onChange}>
                        <SelectTrigger id="pv-supplierId">
              ...
              {watchedPartyType === "CUSTOMER" && (
                <FormField label={t("sales.customer")} htmlFor="pv-customerId" error={partyTypeError}>
                  <Controller
                    control={form.control}
                    name="customerId"
                    render={({ field }) => (
                      <Select value={field.value} onValueChange={field.onChange}>
                        <SelectTrigger id="pv-customerId">
              ...
              {watchedPartyType === "OTHER" && (
                <FormField label={t("banking.otherAccount")} htmlFor="pv-otherAccountId" error={partyTypeError}>
                  <Controller
                    control={form.control}
                    name="otherAccountId"
                    render={({ field }) => (
                      <Select value={field.value} onValueChange={field.onChange}>
                        <SelectTrigger id="pv-otherAccountId">
              ...
              <FormField label={t("banking.description")} htmlFor="pv-description">
                <Input id="pv-description" {...form.register("description")} />
              </FormField>
            </DialogBody>
            <DialogFooter>
