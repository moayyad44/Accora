...
export function ApAgingPage() {
  ...
  const query = useQuery({
    ...
    queryFn: () => accountingApi.reports.apAging(asOfDate),
  });

  return (
    <div>
      <PageHeader
        title={t("accounting.apAging")}
        description={t("accounting.apAgingDesc")}
        breadcrumbs={[{ label: t("nav.accounting") }, { label: t("accounting.apAging") }]}
        actions={
      ...
      <div className="mb-4 flex items-end gap-3">
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="ap-date">{t("accounting.asOfDate")}</Label>
          <Input id="ap-date" type="date" value={asOfDate} onChange={(e) => setAsOfDate(e.target.value)} className="w-48" />
        </div>
      </div>
      ...
      ) : query.data.bySupplier.length === 0 ? (
